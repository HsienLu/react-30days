...
function Day1() {
  ...
  useEffect(() => {
    const countryList = isoCode.map((v) => {
      return {isoCode: v.isoCode, country: v.name[0].text};
    });

    setCountryList(countryList);
    fetch(
      `https://openholidaysapi.org/PublicHolidays?countryIsoCode=${country}&validFrom=${validFrom}&validTo=${validTo}`,
      {
        method: "GET",
      }
    )
  ...
  return (
    <>
      ...
      >
        {countryList.map((v) => {
          return (
            <option key={v.country} value={v.isoCode}>
              {v.country}
            </option>
          );
        })}
        ...
        {holidaysList.map((v) => {
          return (
            <div key={v.id}>
              ...
              {v.name.find((name) => name.language === "EN")?.text}
            </div>
          );
        })}
      </ul>
